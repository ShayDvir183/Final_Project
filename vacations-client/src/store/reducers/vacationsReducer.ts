...
export const vacationsSlice = createSlice({
    ...
    reducers: {
        setVacations: (state: VacationsState, action: PayloadAction<Array<IVacation>>) => {
            ...
            setChartData: (state: VacationsState, action: PayloadAction<Array<any>>) => {
                state.chartData = action.payload;
                },
        setFollowedVacations: (state: VacationsState, action: PayloadAction<Array<IVacation>>) => {
            state.followedVacations = action.payload.filter((vac: IVacation) => { return vac.isFollowed })
        },
        setAdminDialogOpen: (state: VacationsState, action: PayloadAction<IDialog>) => {
            state.adminDialogOpen = action.payload;
        },
        setPaginationVacations: (state: VacationsState, action: PayloadAction<number>) => {
            state.page = action.payload;
            let start = (action.payload -1) * 10;
            const end = start + 10;
            state.paginationVacs = state.vacations.slice(start,end)
            state.followedVacations = state.paginationVacs.filter((vac: IVacation) => { return vac.isFollowed })
        },
    },
});
export const { setVacations,setChartData, setAdminDialogOpen, setFollowedVacations,setPaginationVacations } = vacationsSlice.actions;
